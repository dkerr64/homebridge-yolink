...
async function handleSet(this: YoLinkPlatformAccessory, device: YoLinkDevice, value: CharacteristicValue): Promise<void> {
  ...
  try {
    ...
    const msg = (e instanceof Error) ? e.stack : e;
    platform.log.error('Error in GarageDoor handleSet' + platform.reportError + msg);
  } finally {
    releaseSemaphore();
  }
}
      ...
      case 'DoorSensor':
        ...
        switch (event[1]) {
          ...
          case 'StatusChange':
            ...
            this.currentState = (message.data.state === 'open')
              ? platform.api.hap.Characteristic.CurrentDoorState.OPEN
              : platform.api.hap.Characteristic.CurrentDoorState.CLOSED;
            this.garageService
              .updateCharacteristic(platform.Characteristic.CurrentDoorState, this.currentState);
            this.garageService
